...
StyledRect {
    ...
    // The "default" action id is a freedesktop.org notification-spec convention: senders
    // (e.g. Chromium/Brave web notifications) use it to mean "invoke this when the user
    // clicks the notification body", not as a labelled button.
    readonly property var defaultAction: modelData.actions.find(a => a.identifier === "default") ?? null
    readonly property var visibleActions: modelData.actions.filter(a => a.identifier !== "default")
    ...
    MouseArea {
        ...
        onClicked: event => {
            ...
            const actions = root.visibleActions;
            if (actions.length === 1)
                actions[0].invoke();
        }
        ...
        Item {
            ...
            ButtonRow {
                ...
                IconButton {
                    isRound: true
                    shapeMorph: true
                    fillWidth: root.visibleActions.length === 0
